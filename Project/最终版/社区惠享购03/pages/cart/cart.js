  ...
  async toBuyClick(){
    ...
    if(this.message.length!=0){
      wx.showModal({
        ...
        success:(res)=>{
          if(res.confirm){
            for(var k=0;k<this.message.length;k++){
              var shop;
              for(var p=0;p<this.data.dgs.length;p++){
                if(this.message[k].gId==this.data.dgs[p].gid){
                  shop=this.data.dgs[p].gsid
                }
              }
              console.log(shop);
              wx.request({
                url:'http://49.234.225.248:4399/dadd',
                data: {
                  usid: this.orderList[0].ordernum,
                  gid:this.message[k].gId,
                  dsum:this.message[k].num,
                  dstate:1,
                  dma:this.orderList[0].thm,
                  dtime:this.orderList[0].time,
                  sid:shop
                },
                method: 'POST',
            ...
            //console.log("整体列表：",this.data.CartDataList);
            //console.log("购买列表",listall);
            let list = this.data.CartDataList;
            //购买完就在购物车中删除，使用gid
            for(let i=0;i<listall.length;i++){
              var gid0 = listall[i].gId;
              //console.log(gid0)
              let n = 0;
              for( n ;n<list.length;n++){
                if(list[n].gId === gid0){
                  //console.log(list[n].gId)
                  list.splice(n,1);
                  break;
                }
              }
